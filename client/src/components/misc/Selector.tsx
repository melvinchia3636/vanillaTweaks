...
function Selector({
  ...
  return (
    <div className="w-full flex-1 bg-[#696969] shadow-lg rounded-lg overflow-hidden flex flex-col">
            ...
            <div className="flex flex-col gap-2">
              {packs.map((pack) => (
                <div key={pack.name} className={`flex items-center gap-2 ml-6 text-white ${isIncompatible(selected, pack) ? 'text-red-500' : 'text-white'}`}>
                  <Icon icon="uil:direction" className="w-6 h-6 flex-shrink-0" />
                  <img src={`https://vanillatweaks.net/assets/resources/icons/${name}/1.${version}/${pack.name}.png`} alt={pack.name} className="w-5 h-5" />
      ...
      <button type="button" onClick={() => download(selected, name, version)} className="bg-[#E99743] text-white w-full py-3">Download</button>
    </div>
  );
}
