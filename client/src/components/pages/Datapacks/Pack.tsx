...
function PackItem({pack, category}: { pack: Datapack, category: string }) {
	...
	return (
		...
		>
			<div className={`px-4 bg-neutral-800 w-full text-sm z-[9999] rounded-md break-words text-white absolute bottom-0 overflow-auto left-1/2 -translate-x-1/2 transition-all duration-500 ${itemHover ? 'max-h-full py-4' : 'max-h-0'}`} dangerouslySetInnerHTML={{__html: pack.description}}></div>
